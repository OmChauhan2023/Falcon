...
def compute_features(df):
    ...
    # Volume Ratio (Since intraday volume varies, comparing to previous day's same time is better, 
    # but strictly following user rule of Volume_Ratio > 1.5)
    df['vol_ma_20'] = df['volume'].rolling(20).mean()
    df['volume_ratio'] = df['volume'] / df['vol_ma_20'].shift(1)
    
    # Highest High / Lowest Low of last 4 bars
    df['highest_high_4'] = df['high'].rolling(4).max()
    df['lowest_low_4'] = df['low'].rolling(4).min()
...
def run_backtest(df):
    ...
    df['prev_hh4'] = df['highest_high_4'].shift(1)
    df['prev_ll4'] = df['lowest_low_4'].shift(1)
    df['prev_atr_rising'] = df['atr_rising'].shift(1)
    ...
    for i in range(len(df)):
        ...
        if not in_position:
            # Entry window
            if time_obj >= pd.to_datetime('14:15').time() or time_obj <= pd.to_datetime('09:30').time():
                continue
            ...
            if row['prev_vol_ratio'] < 1.5:
                continue
            if not row['prev_atr_rising']:
                continue
                
            # Long Entry
            if row['prev_close'] > row['prev_hh4']:
                in_position = True
            ...
            # Short Entry
            elif row['prev_close'] < row['prev_ll4']:
                in_position = True
                entry_price = row['open'] - slippage_pts
                target = entry_price - (0.75 * row['atr'])
                stop = entry_price + (0.50 * row['atr'])
...
def generate_report(trades_df, output_dir):
    ...
    print("\n--- Strategy 2: BankNifty Volatility Expansion ---")
    ...
    ax1.set_title('BankNifty Volatility Expansion: Cumulative P&L')
    ax1.set_ylabel('INR')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'strategy2_vol_pnl.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    generate_report(trades, "outputs/v2/strategy2_vol")
